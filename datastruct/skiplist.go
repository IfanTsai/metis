...
// Insert inserts the new node with the specified score and value into the skiplist.
// Assumes the element does not already exist in the skiplist (up to the caller to enforce that).
func (s *Skiplist) Insert(score float64, value string) *SkiplistNode {
	...
	x = newSkiplistNode(level, score, value)
...
// Delete deletes the element with the specified score and value from the skiplist.
// Returns the deleted node if found, otherwise nil.
func (s *Skiplist) Delete(score float64, value string) *SkiplistNode {
	update := make([]*SkiplistNode, maxLevel)
	...
	if x != nil && score == x.Score && value == x.Value {
...
func (s *Skiplist) GetRank(score float64, value string) int64 {
	rank := int64(0)
...
func newSkiplistNode(level int8, score float64, value string) *SkiplistNode {
	return &SkiplistNode{
		Value:    value,
		Score:    score,
		Backward: nil,
